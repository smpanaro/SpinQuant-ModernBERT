"""
Original version of this file comes from transformers===4.48.1

Changes include:
- Fixing the imports to prefix with `transformers`
- Importing and replacing Linear layers with QuantizeLinear, except for LM Head.
- Addition of residual_adjustment_proj to allow first layer to be rotated correctly.
- Thread through R1 and R2.
"""
...
class ModernBertEmbeddings(nn.Module):
    ...
    def rotated_embeddings(self, input_ids: torch.LongTensor, R1=None) -> torch.Tensor:
        input_embeds = self.tok_embeddings(input_ids)
        if R1 is not None:
            dtype = input_embeds.dtype
            input_embeds = (input_embeds.to(torch.float64) @ R1.to(torch.float64)).to(
                dtype
            )
        return input_embeds

    @torch.compile(dynamic=True)
    def compiled_embeddings(self, input_ids: torch.LongTensor, R1=None) -> torch.Tensor:
        return self.drop(self.norm(self.rotated_embeddings(input_ids, R1=R1)))

    def forward(
        self, input_ids: torch.LongTensor = None, inputs_embeds: Optional[torch.Tensor] = None, R1=None
    ) -> torch.Tensor:
        if inputs_embeds is not None:
            hidden_states = self.drop(self.norm(inputs_embeds))
        else:
            hidden_states = (
                self.compiled_embeddings(input_ids, R1=R1)
                if self.config.reference_compile
                else self.drop(self.norm(self.rotated_embeddings(input_ids, R1=R1)))
            )
...
class ModernBertMLP(nn.Module):
    ...
    def __init__(self, config: ModernBertConfig):
        super().__init__()
        self.config = config
        self.Wi = QuantizeLinear(config.hidden_size, int(config.intermediate_size) * 2, bias=config.mlp_bias)
        self.act = ACT2FN[config.hidden_activation]
        self.drop = nn.Dropout(config.mlp_dropout)
        self.Wo = QuantizeLinear(config.intermediate_size, config.hidden_size, bias=config.mlp_bias)

    def forward(self, hidden_states: torch.Tensor, R1=None) -> torch.Tensor:
        input, gate = self.Wi(hidden_states, R1).chunk(2, dim=-1)
        return self.Wo(self.drop(self.act(input) * gate), R1, transpose=True)
...
class ModernBertAttention(nn.Module):
    ...
    def __init__(self, config: ModernBertConfig, layer_id: Optional[int] = None):
        ...
        self.Wqkv = QuantizeLinear(config.hidden_size, 3 * self.all_head_size, bias=config.attention_bias)
        ...
        self.Wo = QuantizeLinear(config.hidden_size, config.hidden_size, bias=config.attention_bias)
    ...
    def forward(
        ...
        **kwargs,
        ...
        qkv = self.Wqkv(hidden_states, R1)
        # TODO: Add support for R2.
        ...
        hidden_states = self.out_drop(self.Wo(hidden_states, R1, transpose=True))

        return (hidden_states,) + attn_outputs[1:]  # add attentions if outputted

class IgnoreArgsIdentity(nn.Module):
...
class ModernBertEncoderLayer(nn.Module):
    def __init__(self, config: ModernBertConfig, layer_id: Optional[int] = None):
        ...
        if layer_id == 0:
            ...
            self.residual_adjustment_proj = QuantizeLinear(config.hidden_size, config.hidden_size, bias=False)
            self.residual_adjustment_proj.weight.data = torch.eye(config.hidden_size, dtype=self.residual_adjustment_proj.weight.dtype)
        else:
            self.attn_norm = nn.LayerNorm(config.hidden_size, eps=config.norm_eps, bias=config.norm_bias)
            self.residual_adjustment_proj = IgnoreArgsIdentity()
        self.attn = ModernBertAttention(config=config, layer_id=layer_id)
        self.mlp_norm = nn.LayerNorm(config.hidden_size, eps=config.norm_eps, bias=config.norm_bias)
        self.mlp = ModernBertMLP(config)

    @torch.compile(dynamic=True)
    def compiled_mlp(self, hidden_states: torch.Tensor, R1=None) -> torch.Tensor:
        return self.mlp(self.mlp_norm(hidden_states), R1=R1)
    ...
    def forward(
        ...
        hidden_states = self.residual_adjustment_proj(hidden_states, R1, residual=True) + attn_outputs[0]
        mlp_output = (
            self.compiled_mlp(hidden_states, R1=R1)
            if self.config.reference_compile
            else self.mlp(self.mlp_norm(hidden_states), R1=R1)
        )
...
class ModernBertModel(ModernBertPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = self.embeddings(input_ids=input_ids, inputs_embeds=inputs_embeds, R1=R1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.R1 is not None:
            ...
            last_hidden_state = (
                last_hidden_state.to(torch.float64) @ self.R1.weight.T.to(torch.float64)
            ).to(dtype)
